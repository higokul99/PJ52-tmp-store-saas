...
<IfModule mod_rewrite.c>
    ...
    # 2. Serve static files from the React frontend (if they exist)
    RewriteCond %{REQUEST_URI} !^/api [NC]
    RewriteCond %{DOCUMENT_ROOT}/code/frontend/dist%{REQUEST_URI} -f [OR]
    RewriteCond %{DOCUMENT_ROOT}/code/frontend/dist%{REQUEST_URI} -d
